...
function kmpTable($list, $c){
    ...
    if ($c>1){
        ...
        $cnd = 0;
        while($pos<$c){
            if ($list[$pos-1] == $list[$cnd]){//substring continue
                $T[$pos] = $cnd + 1;
                $cnd++;
                $pos++;
            }else if ($cnd>0){//roll back
                $cnd = $T[$cnd];
            }else{
                $T[$pos] = 0;
                $pos++;
 ...
 * $W search string
 */
function kmpSearch($S, $C, $W, $c, $T){
    $m = 0; // $S current position
    $i = 0; // $W current position
    $position = array();
    while($m+$i < $C){
        if ($W[$i] == $S[$m+$i]){
            if ($i == $c - 1){
...
function findSubGrid($G, $R, $C, $P, $r, $c, $T){
    $goon = true;
    for($i=0; $i<($R-$r+1) && $goon; $i++){
        $kmpres = kmpSearch($G[$i], $C, $P[0], $c, $T);
        //echo "kmp".$G[$i]." ".$P[0]." ".$kmpres."\n";
        if (is_array($kmpres)){
            $klen = count($kmpres);
            for ($m=0; $m<$klen; $m++){
                $curm = $kmpres[$m];
                for ($j=1; $j<$r; $j++){
                    for ($k=0; $k<$c; $k++){
                        if ($G[$i+$j][$curm+$k] == $P[$j][$k]){
                            //echo $G[$i+$j][$curm+$k]." ".$P[$j][$k]."\n";
                            continue;
                        }else{
                            break;
                        }
